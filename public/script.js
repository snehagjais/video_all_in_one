...
const addVideoStream = (video, stream) =>{      //this help to show and append or add video to user side
  ...
  if(videoGrid.firstElementChild == null) {
	  ...
	  var index = getEmptyParticipantIndex();
	  var participant = document.getElementsByClassName("participant")[index];
...
const showHideWhiteBoard = (action) =>{
    console.log("whiteboard to open");
    socket.emit('whiteBoard', action);
}

socket.on('openWhiteboard', (action) =>{
    if(action == "open") {
